...
/** A warp drive provides the power to make a warp jump.
  *
  * It's just an execution context, but gets passed around in the explicit
  * context instead of the implicit one.
  */
...
object Drive:
    ...
    extension (drive: Drive)
        /** Converts a warp drive back into an [[ExecutionContext]]. This is
          * often helpful when dealing with the underlying [[Future]] values in
          * a [[Warp]].
          */
...
/** A warp provides a mechanism to perform a jump from the input type [[A]] to
  * the output type [[B]].
  *
  * The jump will take an indefinite amount of time, therefore the warp path
  * concludes at a [[Future[B]]] instead of a standard [[B]] value.
  *
  * @param path
...
case class Warp[-A, +B](path: A => Future[B], drive: Drive):
    ...
    /** Appends a move step to the end of the warp path. A [[move]] is the most
      * basic way to transform a warp path.
      *
      * @param move
    ...
    /** Appends a follow step to the end of the warp path. A [[follow]] step is
      * more complicated than a [[move]], in that it can take an indefinite
      * amount of time.
      *
      * @param follow
      *   a function which travels from a value [[B]] to some eventual value
      *   [[C]].
      * @return
      *   a new warp with an updated path
      */
    ...
    /** Performs an additional warp after the current warp is completed. Aka a
      * "double warp".
      *
      * @param warp
    ...
    /** Calculates the next warp to perform once the current warp is completed.
      ...
      * Also, requires a warp that can occur from anywhere
      * ([[Warp.FromAnywhere]]).
      *
      * @param calculate
    ...
    /** The starting location is all set and we are ready to jump.
      ...
      * @return
      *   a Scala [[Future]] which yields the final location of the warp once
      *   the jump is completed
      */
    def jump(start: A): Future[B] =
        path(start)

    /** Swaps the warp drive with the provided one. The warp will use the
      * provided drive when making a jump.
      *
      * @param drive
...
object Warp:
    ...
    /** Defines a starting location for the warp jump. The ending location of
      * the jump will be the same as the starting location.
      *
      * Often used to create a foundational warp to start building off of.
      */
    def startAt[A]: Warp[A, A] =
        new Warp(a => Future.fromTry(Try(a)), Drive.standard)
    ...
    extension [A, B](listWarp: Warp[A, List[B]])
        /** Splits the universe into many pieces and runs warp jumps from each
          * location the initial warp sends you to.
          */
        def multiWarp[C](warp: Warp[B, C]): Warp[A, List[C]] =
            ...
            listWarp
                .follow: list =>
                    list
                        .map(warp.jump)
                        .foldRight(Future.successful(List.empty[C])):
                            case (nextFuture, futureList) =>
                                nextFuture
                                    .flatMap(next => futureList.map(next :: _))
